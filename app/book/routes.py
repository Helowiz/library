...
def add_book():
    ...
    if form.validate_on_submit():
        ...
        if book := Book.find_duplicate(
            form.title.data, form.author_id.data, form.format.data
        ):
        ...
        try:
            ...
            new_book.series_id = (
                form.series_id.data if form.series_id.data != 0 else None
            )
            if form.tags.data:
...
def update_status(book_id):
    ...
    if status_key in ReadingStatus.__members__:
        ...
        if latest_session is None:
            ...
            flash(
                f'Le statut du livre a été mis à jour : "{new_status.value}"', "success"
            )
...
def update_progress(book_id):
    ...
    if book.number_of_pages and (pages_read < 0 or pages_read > book.number_of_pages):
        flash(
            "Le nombre de pages lues doit être entre 0 et le nombre total de pages du livre.",
            "danger",
        )
        return redirect(url_for("book.detail_book", book_id=book.id))
